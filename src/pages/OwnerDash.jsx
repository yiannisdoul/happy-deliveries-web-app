...
import { CheckCircle, XCircle, Calendar, AlertCircle, Bell, Minimize2, User, Camera, PenTool, UploadCloud } from 'lucide-react'; 
...
export default function OwnerDash() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      docs.sort((a, b) => {
        // Sort: Pending -> Accepted -> Delivered -> Rejected
  ...
  const handleSubmitDelivery = async (e) => {
    ...
    if (!photoFile) return alert("Please take a photo of the delivery");

    setIsSubmitting(true);
    try {
      // 1. Upload Photo
      const photoRef = ref(storage, `proofs/${deliveringJobId}/photo_${Date.now()}.jpg`);
      await uploadBytes(photoRef, photoFile);
      const photoURL = await getDownloadURL(photoRef);

      // 2. Upload Signature (Convert Base64 to Blob)
      const sigData = sigPad.current.getTrimmedCanvas().toDataURL('image/png');
      const sigBlob = await (await fetch(sigData)).blob();
      const sigRef = ref(storage, `proofs/${deliveringJobId}/sig_${Date.now()}.png`);
              ...
              <form onSubmit={handleSubmitDelivery} className="space-y-4">
                 ...
                 <div>
                    ...
                    <input 
                      required
                      type="text" 
                      placeholder="Who accepted the package?"
                 ...
                 {/* 2. Photo Upload */}
                 <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">Proof of Delivery Photo</label>
                       ...
                       <div className="flex flex-col items-center justify-center text-gray-500">
                          {photoFile ? (
                             <div className="text-green-600 font-bold flex items-center">
                               <CheckCircle className="h-5 w-5 mr-1"/> Photo Selected
                             </div>
                          ) : (
                             <>
                 ...
                 {/* 3. Signature */}
                 <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">Receiver Signature</label>
                 ...
                 <button 
                   ...
                   className="w-full bg-green-600 text-white font-bold py-3 rounded-lg hover:bg-green-700 shadow-lg flex items-center justify-center"
                 >
                   {isSubmitting ? (
                     <span>Uploading...</span> 
                   ) : (
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
              <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-5 animate-fade-in-up border-l-8 border-yellow-400 max-h-[85vh] flex flex-col">
                      ...
                      {editedJobs.map(job => (
                        <li key={job.id} className="text-sm flex items-start">
                          ...
                          <div className="break-words w-full">
                            <span className="font-semibold block">{job.pickupName}</span> 
                ...
                <div className="flex-shrink-0 pt-2">
                    <button onClick={handleCloseNotification} className="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700 transition shadow-lg">Close Message</button>
                </div>
              </div>
            </div>
          ) : (
                ...
                <div className="flex justify-between items-start gap-2">
                  ...
                  {job.status === 'pending' ? (
                    <div className="flex gap-2">
                      <button onClick={() => handleStatus(job.id, 'accepted')} className="p-2 bg-green-100 text-green-600 rounded-full hover:bg-green-200" title="Accept"><CheckCircle className="h-6 w-6" /></button>
                      <button onClick={() => handleStatus(job.id, 'rejected')} className="p-2 bg-red-100 text-red-600 rounded-full hover:bg-red-200" title="Reject"><XCircle className="h-6 w-6" /></button>
                    </div>
                  ) : job.status === 'accepted' ? (
                    <div className="flex flex-col items-end gap-2">
                        ...
                        <button 
                          onClick={() => setDeliveringJobId(job.id)}
                          className="flex items-center bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow hover:bg-blue-700"
                        >
                          <PenTool className="h-3 w-3 mr-1" /> Complete
                        </button>
                    </div>
                  ) : (
                    <span className={`px-3 py-1 text-xs rounded-full font-bold uppercase ${job.status === 'delivered' ? 'bg-slate-200 text-slate-600' : 'bg-red-100 text-red-800'}`}>
                        {job.status}
                    </span>
                  )}
                </div>
